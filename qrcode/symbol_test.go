...
func TestSymbolBasic(t *testing.T) {
	...
	m := newSymbol(symbolSize, quietZoneSize)
	if m.fullSize != symbolSize+quietZoneSize*2 {
		t.Errorf("Symbol size is %d, expected %d", m.fullSize, symbolSize+quietZoneSize*2)
	}

	for i := range symbolSize {
		for j := range symbolSize {
			v := m.get(i, j)
			if v != false {
