...
func writeMarkerDefs(w io.Writer, c *Canvas, scaleX, scaleY int) {
	...
	y := float64(scaleY) / 2
	if c.hasStartMarker {
		_, _ = fmt.Fprintf(w, markerTag, nameStartMarker, x, y, "M 10 0 L 10 10 L 0 5 z")
	}
	if c.hasEndMarker {
		_, _ = fmt.Fprintf(w, markerTag, nameEndMarker, x, y, "M 0 0 L 10 5 L 0 10 z")
	}
}
...
func writeClosedPaths(w io.Writer, c *Canvas, scaleX, scaleY int) {
	...
	for i, obj := range c.objects() {
		...
		if first {
			_, _ = io.WriteString(w, `<g id="closed" stroke="#000" stroke-width="2" fill="none">`)
			first = false
		}
		opts := ""
		if obj.IsDashed() {
		...
		_, _ = fmt.Fprintf(w, pathTag, "closed", i, opts, flatten(obj.Points(), scaleX, scaleY)+"Z")
	}
	if !first {
		_, _ = io.WriteString(w, "</g>")
	}
}
...
func writeOpenPaths(w io.Writer, c *Canvas, scaleX, scaleY int) {
	...
	for i, obj := range c.objects() {
		...
		if first {
			_, _ = io.WriteString(w, `<g id="lines" stroke="#000" stroke-width="2" fill="none">`)
			first = false
		}
		points := obj.Points()
		for _, p := range points {
			...
			case dot:
				...
				_, _ = fmt.Fprintf(w, `<circle cx="%g" cy="%g" r="3" fill="#000" />`, sp.X, sp.Y)
			case tick:
				sp := scale(p, scaleX, scaleY)
				const tickLine = `<line x1="%g" y1="%g" x2="%g" y2="%g" stroke-width="2" />`
				_, _ = fmt.Fprintf(w, tickLine, sp.X-4, sp.Y-4, sp.X+4, sp.Y+4)
				_, _ = fmt.Fprintf(w, tickLine, sp.X+4, sp.Y-4, sp.X-4, sp.Y+4)
			}
		}
		...
		_, _ = fmt.Fprintf(w, pathTag, "open", i, opts, flatten(points, scaleX, scaleY))
	}
	if !first {
		_, _ = io.WriteString(w, "</g>")
	}
...
func writeTexts(w io.Writer, c *Canvas, font string, scaleX, scaleY int) {
	...
	for i, obj := range c.objects() {
		...
		if first {
			_, _ = fmt.Fprintf(w, `<g id="text" stroke="none" style="font-family:%s;font-size:%gpx">`, font, fontSize)
			first = false
		}
